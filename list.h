...
struct ht_list_head {
	struct ht_list_head *next, *prev;
};

struct ht_hlist_head {
...
static inline void HT_INIT_LIST_HEAD(struct ht_list_head *list) {
    list->next = list;
    list->prev = list;
}

static inline void __ht_list_add(struct ht_list_head *new,
                              struct ht_list_head *prev,
                              struct ht_list_head *next) {
    next->prev = new;
    new->next = next;
    new->prev = prev;
    prev->next = new;
...
static inline void ht_list_add(struct ht_list_head *new, struct ht_list_head *head) {
    __ht_list_add(new, head, head->next);
}

static inline void ht_list_add_tail(struct ht_list_head *new, struct ht_list_head *head) {
    __ht_list_add(new, head->prev, head);
}

static inline void __ht_list_del(struct ht_list_head *prev, struct ht_list_head *next) {
    next->prev = prev;
    prev->next = next;
}

static inline void ht_list_del(struct ht_list_head *entry) {
...
#define HT_HLIST_HEAD_INIT { .first = NULL }
...
static inline int ht_hlist_empty(const struct ht_hlist_head *h)
{
	return !h->first;
}
...
static inline void ht_hlist_add_head(struct hlist_node *n, struct ht_hlist_head *h)
{
 ...
 */
static inline void ht_hlist_move_list(struct ht_hlist_head *old,
				   struct ht_hlist_head *new)
{
	new->first = old->first;
	if (new->first)
